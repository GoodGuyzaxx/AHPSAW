...
@section('content')
    <main class="container-fluid px-4 animate-fade-in">
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div>
                ...
                <p class="mb-0 text-muted">Kelola data alternatif dan nilai kriteria untuk setiap siswa.</p>
            </div>
        </div>
            ...
            <div class="card-body p-4">
                    ...
                    <div class="col-md-9">
                        <form action="{{ route('alternatif.index') }}" method="GET">
                            <div class="input-group input-group-sm">
                                <input type="text" name="search" class="form-control" placeholder="Cari berdasarkan nama atau kelas..." value="{{ request('search') }}">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover">
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ 4 + $criterias->count() }}" class="text-center text-danger py-4">
            ...
            <div class="modal-content border-0 shadow-lg">
                <form action="{{ route('alternatif.store') }}" method="POST">
                    ...
                    <div class="modal-header bg-gradient-info text-white border-0">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                ...
                                <small class="opacity-75">Isi data siswa dan nilai kriterianya</small>
                            </div>
                        </div>
                    ...
                    <div class="modal-body p-4">
                        <div class="mb-4">
                            <label for="student_id" class="form-label fw-semibold"><i class="fas fa-user-graduate me-2 text-primary"></i>Pilih Siswa</label>
                            <select class="form-select @error('student_id') is-invalid @enderror" id="student_id" name="student_id" required>
                                <option value="" disabled selected>-- Pilih dari daftar siswa yang tersedia --</option>
                                @forelse ($student_list as $kelas => $students)
                                    <optgroup label="Kelas {{ $kelas }}">
                                        @foreach ($students as $student)
                                            <option value="{{ $student->id }}|{{ $student->kelasId }}">{{ $student->name }}</option>
                                        @endforeach
                                    </optgroup>
                                @empty
                                    <option disabled>Tidak ada siswa yang bisa ditambahkan</option>
                                @endforelse
                            </select>
    ...
    <script>
        function submitForm() {
            ...
            url.searchParams.set('page', '1'); // Reset to page 1 on perPage change
